...
import ChoThueComponent from './ChoThueComponent';
...
const ChoThue: React.FC = () => {
  ...
  useEffect(() => {
    dispatch(getRealEstateListings({ realEstateType: "RENT" }));
  }, [])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await dispatch(postRealEstateListing(formattedFormData) as any);
    
    await dispatch(getRealEstateListings({ realEstateType: "RENT" }));
  ...
  return (
    <div className="container mx-auto">
      ...
      >
        ...
        {showForm ? 'Ẩn form Đăng tin' : 'Đăng tin cho thuê'}
      </button>
        ...
        <div className="max-w-4xl mx-auto mb-4 lg:mb-8">
          ...
          {!isLoggedIn ? (
            ...
            <div className="bg-gradient-to-r from-primary-color to-blue-500 p-6 text-white">
              <h2 className="text-3xl font-bold text-center">Đăng Tin Cho Thuê</h2>
                ...
                <div className="flex space-x-4 mb-6">
                  <div className="flex-1">
                    <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="price">
                      Giá (VNĐ)/tháng <span className="text-red-500">*</span>
                    </label>
                    <input
      ...
      )}
       {
        Array.isArray(realEstateListings) &&
          [...realEstateListings] 
            ...
            .map((listing) => (
              <ChoThueComponent
              images={listing.mediaList.map(media => media.mediaUrl)}
              title={listing.title}
              price={convertCurrency(listing.price)}
              area={listing.area}
      ...
      <div className="flex justify-center mt-4">
        <button onClick={prevPage} disabled={currentPage === 1} className="mx-1 px-4 py-2 rounded bg-gray-200 cursor-pointer">
          Previous
        </button>
        
        {Array.from({ length: Math.ceil(realEstateListings.length / itemsPerPage) }, (_, index) => (
          <button
            key={index + 1}
        ...
        <button onClick={nextPage} disabled={currentPage === Math.ceil(realEstateListings.length / itemsPerPage)} className="mx-1 px-4 py-2 rounded bg-gray-200 cursor-pointer">
          Next
        </button>
      </div>
    </div>
